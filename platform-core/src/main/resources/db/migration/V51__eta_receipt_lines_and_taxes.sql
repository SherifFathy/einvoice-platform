-- V51: ETA receipt lines and line taxes tables
-- Wave 7 — ETA Document Tables and Submission Engine (data-model.md §5–6)

CREATE TABLE eta_receipt_lines (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    header_id       UUID NOT NULL REFERENCES eta_receipt_headers(id) ON DELETE CASCADE,
    ...
    CONSTRAINT uq_eta_receipt_line
        UNIQUE (header_id, line_number),

    CONSTRAINT chk_eta_rec_line_item_type
        CHECK (item_type IN ('GS1','EGS')),

    CONSTRAINT chk_eta_rec_line_quantity
        CHECK (quantity > 0)
);

CREATE TABLE eta_receipt_line_taxes (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    line_id         UUID NOT NULL REFERENCES eta_receipt_lines(id) ON DELETE CASCADE,
