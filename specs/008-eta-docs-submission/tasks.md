...
- [ ] T113 [P] Run `mvn -pl platform-core,platform-security,platform-eta,platform-api clean verify` from repo root; resolve any test or build failures. No `-DskipTests`. *(Status 2026-05-17: platform-core, platform-security, and platform-eta now green at commit `b0f4da5` after three previously-undetected test bugs were fixed; platform-api has 48 failures + 2 errors in 12 test classes related to MockMvc auth context — deferred for separate triage, see `perf-results.md` for details.)*
