...
4. **Given** the user is configuring for an environment that supports a token name and password (e.g., Production), **When** they save without providing those values, **Then** the system either accepts blanks (if optional for that environment) or rejects with a clear message about which fields are required.
...
- A user's active context changes mid-session (e.g., switches environment); subsequent reads and writes immediately apply to the new context and previous-context entries become invisible without page refresh.
...
- A user attempts to delete a customer or item that has been referenced by a draft, submitted, or historical invoice in later waves; deletion is refused or the record is soft-deactivated rather than removed (see assumptions).
...
- **FR-005**: A customer entry MUST capture at minimum: an English name (required), an Arabic name (optional), a customer type, a tax or VAT identifier, an identification document type and value, structured address information, and contact email and phone.
...
- **FR-007**: The system MUST support marking customers as inactive without permanent deletion, and MUST exclude inactive customers from default list views unless the user opts to show them.

#### Items (ETA and ZATCA)

- **FR-008**: Authorized users MUST be able to create, view, and update ETA item entries and ZATCA item entries, and to deactivate them, scoped to a company and authority environment.
...
- **FR-010**: A ZATCA item entry MUST capture at minimum: an internal code (required), an English name (required), an Arabic name (optional), a unit type, a unit price, a VAT category that is one of `S`, `Z`, `E`, `O`, and a VAT rate; an item code is optional.
...
- **FR-013**: An ETA configuration MUST capture at minimum: client identifier, two client secrets, an optional token name and token password (required only for environments that need them), a token URL, a submission URL, and optional point-of-sale device fields (serial, operating system version, model).
- **FR-014**: The system MUST update timestamps on configuration changes so that "last modified" is observable.
...
- **FR-016**: A ZATCA configuration MUST capture at minimum: a private key, a device identifier, a certificate signing request, a compliance certificate and compliance API secret (required), and an optional production certificate, production API secret, and certificate expiry date.
...
- **SC-008**: A user with multiple company assignments switching their active environment sees the customers and items lists update to show only the new environment's data within one page load, with no leakage from the previous environment.
...
- "Delete" of a customer or item is preferred to be expressed as deactivation when the entry has historical references, to preserve audit and reporting; hard delete is only safe before any document references the entry. The user interface presents this consistently as "deactivate" once references exist.
...
- Branches are optional on a configuration record. If a configuration is tied to a specific branch, this scoping is informational for now (used by reporting and downstream flows) and does not further partition customers or items.
