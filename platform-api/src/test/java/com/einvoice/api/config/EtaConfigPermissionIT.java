...
class EtaConfigPermissionIT {
    ...
    void setUp() {
        Company company = companyRepository.save(Company.builder()
                .nameEn("Perm Config Co").nameAr("شركة").taxNumber("PCFG001").isActive(true).build());
    ...
    void viewerCannotReadConfig() throws Exception {
        User viewer = User.builder()
                .name("Viewer")
                .email("viewer@eta-config-perm.com")
    ...
    void accountantCannotReadOrWriteConfig() throws Exception {
        User accountant = User.builder()
                .name("Accountant")
                .email("accountant@eta-config-perm.com")
                .passwordHash(passwordEncoder.encode("P"))
                .isSuperUser(false)
                .isActive(true)
                .build();
    ...
    void companyAdminCanReadAndWriteConfig() throws Exception {
        User admin = User.builder()
                .name("Admin")
                .email("admin@eta-config-perm.com")
                .passwordHash(passwordEncoder.encode("P"))
                .isSuperUser(false)
                .isActive(true)
                .build();
        ...
        mockMvc.perform(put("/api/companies/{companyId}/eta/config", companyId)
                        ...
                        .content(objectMapper.writeValueAsString(body)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.clientId").value("admin-client"));
    }
    ...
    void adminMode_rejectedWithCompanyContextRequired() throws Exception {
        User superUser = User.builder()
                .name("Super")
                .email("super@eta-config-perm.com")
                .passwordHash(passwordEncoder.encode("P"))
                .isSuperUser(true)
                .isActive(true)
                .build();
    ...
    private void seedConfigAdminRole(User user, UUID compId) {
        TransactionRole role = transactionRoleRepository
                .findByAuthorityAndTransactionTypeAndRoleCode("ETA", "CONFIG", "ADMIN")
                .orElseGet(() -> {
                    TransactionRole r = transactionRoleRepository.save(TransactionRole.builder()
                            .authority("ETA")
                            .transactionType("CONFIG")
                            .roleCode("ADMIN")
                            .description("ETA Config Admin")
                            .build());
                    transactionRolePermissionRepository.save(TransactionRolePermission.builder()
                            .transactionRole(r)
                            .permissionCode("VIEW")
