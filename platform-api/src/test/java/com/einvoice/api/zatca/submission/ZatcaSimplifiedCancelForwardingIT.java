...
@Testcontainers
class ZatcaSimplifiedCancelForwardingIT {
    ...
    void setUp() {
        Company company = companyRepository.save(Company.builder()
                .nameEn("Simp Cancel Co").nameAr("اختبار")
                .taxNumber("SC001").isActive(true).build());
        companyId = company.getId();

        User user = User.builder()
                .name("Test Admin")
                .email("simp-cancel@test.com")
    ...
    @Test
    void cancelAcceptedSimplifiedForwardsReasonAndReflectsStatus()
            throws Exception {
        var line = Map.of(
                "lineNumber", 1,
        ...
        String body = objectMapper.writeValueAsString(Map.of(
                "invoiceNumber", "SMP-CN-" + System.nanoTime(),
                "invoiceTypeCode", "388",
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.reportingStatus")
                        .value("REPORTED"));
        ...
        mockMvc.perform(
                        post("/api/companies/{companyId}"
                                ...
                                .header("Authorization",
                                        "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"reason\":\"Return processed\"}"))
                .andExpect(status().isOk())
        ...
        verify(engine).cancel(argThat(
                (CancelInput in) -> "Return processed"
                        .equals(in.reason())));
    }
}
