...
class ZatcaCancelForwardingIT {
    ...
    void setUp() {
        Company company = companyRepository.save(Company.builder()
                .nameEn("Cancel Test Co").nameAr("اختبار")
                .taxNumber("CT001").isActive(true).build());
    ...
    @Test
    void cancelAcceptedStandardForwardsToZatcaAndReflectsStatus()
            throws Exception {
        var line = Map.of(
                "lineNumber", 1,
                "description", "Service",
        ...
        String body = objectMapper.writeValueAsString(Map.of(
                "invoiceNumber", "STD-CN-" + System.nanoTime(),
                "invoiceTypeCode", "388",
        ...
        String zatcaUuid = UUID.randomUUID().toString();
        ...
        when(engine.cancel(any())).thenReturn(new AuthorityResponse(
                true, "CANCELLED", null, null,
                null, 200, null,
                "{\"status\":\"CANCELLED\"}"));
        ...
        mockMvc.perform(
                        post("/api/companies/{companyId}"
                                ...
                                .header("Authorization",
                                        "Bearer " + token)
                                .contentType(MediaType.APPLICATION_JSON)
                                .content("{\"reason\":\"Wrong buyer\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.state").value("CANCELLED"))
                .andExpect(jsonPath("$.clearanceStatus")
                        .value("CANCELLED"));

        verify(engine).cancel(argThat(
                (CancelInput in) -> "Wrong buyer".equals(
                        in.reason())));

        String attemptsJson = mockMvc.perform(
        ...
        var attempts = objectMapper.readTree(attemptsJson);
        boolean found = false;
        for (int i = 0; i < attempts.size(); i++) {
            var a = attempts.get(i);
            if (a.path("attemptNumber").asInt() == 2) {
                assertEquals("SUCCESS", a.path("result").asText(),
                        "Cancel attempt (attemptNumber=2) should have result=SUCCESS");
                found = true;
            }
        }
        assertEquals(true, found,
                "Expected an attempt with attemptNumber=2");
    }
}
