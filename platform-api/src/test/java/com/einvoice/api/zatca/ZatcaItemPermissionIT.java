...
class ZatcaItemPermissionIT {
    ...
    void setUp() {
        Company company = companyRepository.save(Company.builder()
                .nameEn("Perm Co").nameAr("شركة").taxNumber("ITEM_PERM_ZATCA").isActive(true).build());
    ...
    void viewerCanGetButCannotPostPutDelete() throws Exception {
        User viewer = User.builder()
                .name("Viewer")
                .email("viewer@zatca-item-perm.com")
        ...
        mockMvc.perform(put("/api/companies/{companyId}/zatca/items/{itemId}",
    ...
    void adminMode_rejectedWithCompanyContextRequired() throws Exception {
        User superUser = User.builder()
                .name("Super")
                .email("super@zatca-item-perm.com")
                .passwordHash(passwordEncoder.encode("P"))
                .isSuperUser(true)
                .isActive(true)
                .build();
    ...
    private void seedViewerRole(User user, UUID compId) {
        TransactionRole viewerRole = transactionRoleRepository
                .findByAuthorityAndTransactionTypeAndRoleCode("ZATCA", "ITEMS", "VIEWER")
                .orElseGet(() -> {
                    TransactionRole role = transactionRoleRepository.save(TransactionRole.builder()
                            .authority("ZATCA")
                            .transactionType("ITEMS")
                            .roleCode("VIEWER")
                            .description("Test ZATCA Items Viewer")
                            .build());
    ...
    @Test
    void accountantCanCreateAndViewButNotDelete() throws Exception {
        User accountant = User.builder()
                .name("Accountant")
                .email("accountant@zatca-item-perm.com")
                .passwordHash(passwordEncoder.encode("P"))
                .isSuperUser(false)
                .isActive(true)
                .build();
        ...
        seedAccountantRole(accountant, companyId);
        ...
        Map<String, Object> body = Map.of(
                "internalCode", "SKU-ACCT-Z",
                "nameEn", "Accountant Item",
                "vatCategory", "S",
                "vatRate", 15.0);
    ...
    private void seedAccountantRole(User user, UUID compId) {
        TransactionRole accountantRole = transactionRoleRepository
                .findByAuthorityAndTransactionTypeAndRoleCode("ZATCA", "ITEMS", "ACCOUNTANT")
                .orElseGet(() -> {
                    TransactionRole role = transactionRoleRepository.save(TransactionRole.builder()
                            .authority("ZATCA")
                            .transactionType("ITEMS")
                            .roleCode("ACCOUNTANT")
                            .description("Test ZATCA Items Accountant")
                            .build());
                    transactionRolePermissionRepository.save(TransactionRolePermission.builder()
                            .transactionRole(role)
                            .permissionCode("VIEW")
