...
@Component({
  selector: 'app-standard-number-dialog',
  standalone: true,
...
export class StandardNumberDialogComponent {
  invoiceNumber = '';
}

@Component({
  selector: 'app-standard-cancel-reason-dialog',
  standalone: true,
  imports: [CommonModule, MatDialogModule, MatButtonModule,
            MatFormFieldModule, MatInputModule, FormsModule],
  template: `
...
export class StandardCancelReasonDialogComponent {
  reason = '';
}
...
export class ZatcaStandardDetailComponent {
  ...
  artifactTypes = ['UBL_XML', 'SIGNED_UBL_XML', 'QR_PNG', 'CLEARED_XML', 'ZATCA_REQUEST', 'ZATCA_RESPONSE'];
  ...
  document = toSignal(
    ...
    })), { initialValue: null as unknown as HttpResponse<ZatcaStandardDocument> }
  );
  ...
  cancel(): void {
    ...
    if (!doc) return;
    const companyId = this.context()?.activeCompanyId ?? '';
    const ref = this.dialog.open(StandardCancelReasonDialogComponent, {
      width: '400px',
    });
  ...
  cloneAsDraft(): void {
    ...
    if (!doc) return;
    const companyId = this.context()?.activeCompanyId ?? '';
    const ref = this.dialog.open(StandardNumberDialogComponent, {
      width: '400px',
    });
    ref.afterClosed().subscribe(newNumber => {
      if (newNumber) {
        this.service.cloneAsDraft(companyId, doc.id, newNumber).subscribe({
          next: resp => this.router.navigate(['/standard', resp.body?.id]),
