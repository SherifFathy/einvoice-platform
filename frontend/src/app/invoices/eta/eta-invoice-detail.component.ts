...
import { SubmissionHistoryComponent } from '../shared/submission-history.component';
import { ArtifactDownloadComponent } from '../shared/artifact-download.component';
...
export class EtaInvoiceDetailComponent {
  ...
  getArtifactUrlFn(): (type: string) => string {
    ...
    if (!inv) return () => '';
    const companyId = this.context()?.activeCompanyId ?? '';
    return (type: string) => this.service.getArtifactUrl(companyId, inv.id, type);
  }
