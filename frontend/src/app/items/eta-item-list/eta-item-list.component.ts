...
import { EtaItemService, EtaItemResponse } from '../services/eta-item.service';
...
@Component({
  ...
  templateUrl: './eta-item-list.component.html',
  styleUrls: ['./eta-item-list.component.scss'],
})
export class EtaItemListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = [
    'company', 'internalCode', 'itemType', 'itemCode', 'nameEn', 'nameAr', 'unitPrice', 'taxType', 'taxSubtype', 'taxRate', 'isActive', 'actions',
  ];
  ...
  loadItems(): void {
    ...
    this.itemService.list(
      ...
      error: (err) => this.toast.error(err.error?.error || 'Failed to load ETA items'),
    });
  }
  ...
  onDelete(item: EtaItemResponse): void {
