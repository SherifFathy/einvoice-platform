...
import { EtaCustomerService, EtaCustomerResponse } from '../services/eta-customer.service';
...
@Component({
  selector: 'app-eta-customer-list',
  standalone: true,
  imports: [
  ...
  templateUrl: './eta-customer-list.component.html',
  styleUrls: ['./eta-customer-list.component.scss'],
})
export class EtaCustomerListComponent implements OnInit, OnDestroy {
  ...
  displayedColumns: string[] = [
    'company', 'nameEn', 'nameAr', 'taxNumber', 'customerType', 'isActive', 'actions',
  ];
  ...
  loadCustomers(): void {
    ...
    this.customerService.list(
      ...
      error: (err) => this.toast.error(err.error?.error || 'Failed to load ETA customers'),
    });
  }
  ...
  onDelete(customer: EtaCustomerResponse): void {
